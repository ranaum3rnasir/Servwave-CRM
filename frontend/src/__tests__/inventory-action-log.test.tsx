// Inventory P5 §2 — ActionLogView: filterable movements ledger over the
// {data,meta} envelope. useMovements stays REAL here (over the setup-mocked
// axios) so the camelCase→snake_case param mapping is exercised end-to-end;
// the picker hooks resolve from URL-routed api.get mocks.
...
describe('ActionLogView (P5 §2, QA-705)', () => {
  it('renders the ledger: type pill, item, qty, from→to names, job/invoice links, actor, cost', async () => {
    ...
    expect(screen.getByText('Main Warehouse → —')).toBeInTheDocument(); // consume has no to-location
  ...
  it('each filter control re-queries with the right snake_case param', async () => {
    ...
    // Dates - DatePicker now (typeable text field, ISO also accepted as an escape
    // hatch); it only commits to parent state on blur, unlike the native input this
    // replaced.
    ...
    fireEvent.change(toDateInput, { target: { value: '2026-07-16' } });
  ...
  it('reads ?lo= from the URL, filters by it, and shows a dismissible chip', async () => {
    ...
    await waitFor(() => {
      const params = mockApi.get.mock.calls.find(
        (c) => c[0] === '/api/inventory/movements',
      )![1]!.params;
      expect(params.logistic_order_id).toBe('lo-uuid-1');
    });
    ...
    // Actually dismiss it: the chip (and its × control) must disappear, and
    ...
    await waitFor(() => {
      const params = movementCalls().at(-1)![1]!.params;
      expect(params.logistic_order_id).toBeUndefined();
    });
  });
  ...
  it('CSV export honours the LO filter it claims to snapshot', async () => {
    ...
    await waitFor(() => {
      const exportCall = mockApi.get.mock.calls
        .filter((c) => c[0] === '/api/inventory/movements')
        .at(-1)!;
      expect(exportCall[1]!.params.logistic_order_id).toBe('lo-uuid-1');
      expect(exportCall[1]!.params.limit).toBe(100);
    });
  });
});
