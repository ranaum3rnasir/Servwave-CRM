// TG11 (Q8) — the extensible Unassigned bucket stack: Jobs / Walkthroughs / Service-Plans
// stub render as collapsible sections, cards drag out (page-supplied channels), and the
// stack root is the D6 drag-to-unschedule drop target (grid-event-id only, disabled in
// plan mode). Drag payloads are plain objects mimicking dataTransfer (jsdom has no
// DataTransfer); keys come from the dragChannels codec — same pattern as MemberWeekBoard.
...
describe('UnassignedBuckets — TG11 extensible bucket stack (Q8)', () => {
  ...
  it('collapsing a bucket hides its cards (default expanded)', async () => {
    ...
    // The section animates closed (framer-motion exit), so the card leaves
    // the DOM asynchronously rather than on the same tick as the click.
    await waitFor(() => expect(screen.queryByText('J00041')).not.toBeInTheDocument());
  });
  ...
  // ── SRVW-102 - a bucket the viewer cannot reach is dropped, not explained ──
  ...
  // ── D9 (TG13) — read-only roles: cards are not drag sources ──
  describe('dragDisabled (D9 read-only roles)', () => {
  ...
  // Unscheduled work has no calendar block for the board's search highlight to land on,
  // so the highlight has to land on the bucket card instead.
  describe('search highlight', () => {
    // jsdom ships no scrollIntoView; the highlight effect calls it.
    if (!Element.prototype.scrollIntoView) Element.prototype.scrollIntoView = vi.fn();

    const baseProps = {
      events: [freshJob, walkthrough],
      members,
      onDragStartCard: vi.fn(),
      onUnscheduleDrop: vi.fn(),
      onCardClick: vi.fn(),
    ...
    it('marks only the selected card', () => {
      renderBuckets([freshJob, walkthrough], { highlightedCardId: 'wt-lead-1' });
      expect(screen.getByText('L00012').closest('[draggable]')).toHaveAttribute('data-highlighted');
      expect(screen.getByText('J00041').closest('[draggable]')).not.toHaveAttribute('data-highlighted');
    });

    it('marks nothing when no card is selected', () => {
      renderBuckets([freshJob, walkthrough]);
      expect(screen.getByText('L00012').closest('[draggable]')).not.toHaveAttribute('data-highlighted');
    });

    it('re-opens a bucket the user collapsed, so the highlight is not hidden', () => {
      const { rerender } = renderWithProviders(<UnassignedBuckets {...baseProps} />);
      const header = screen.getByRole('button', { name: /Walkthroughs/ });

      fireEvent.click(header);
      expect(header).toHaveAttribute('aria-expanded', 'false');

      rerender(<UnassignedBuckets {...baseProps} highlightedCardId="wt-lead-1" />);
      expect(header).toHaveAttribute('aria-expanded', 'true');
    });

    it('collapsing while a highlight forces the bucket open still collapses it afterwards', () => {
      const { rerender } = renderWithProviders(
        <UnassignedBuckets {...baseProps} highlightedCardId="wt-lead-1" />,
      );
      const header = screen.getByRole('button', { name: /Walkthroughs/ });

      fireEvent.click(header);
      // The override still wins while it is live...
      expect(header).toHaveAttribute('aria-expanded', 'true');

      // ...but the click is not swallowed: the bucket is collapsed once it lifts.
      rerender(<UnassignedBuckets {...baseProps} highlightedCardId={null} />);
      expect(header).toHaveAttribute('aria-expanded', 'false');
    });
  });
});
