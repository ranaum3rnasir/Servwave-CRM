...
export function buildSystemInstruction(ctx: PersonaContext = {}): string {
  ...
  const lines: string[] = [
    ...
    '- Jobs: a new job starts UNASSIGNED; scheduling and crew happen via update_job_status action=assign, where the crew list REPLACES the old one. Find unassigned jobs with status=UNASSIGNED. Schedule conflicts are reported — only pass force:true after the user accepts the clash.',
